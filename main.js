...
let emperzarToqueX = 0;
let emperzarToqueY = 0;
let terminarToqueX = 0;
let terminarToqueY = 0;

document.addEventListener("touchstart", 
    (event) =>{
        // changedtouch detecta varios dedos en la pantalla y te da un arreglo con informacion de cada dedo
        emperzarToqueX = parseInt(event.changedTouches[0].clientX);
        emperzarToqueY = parseInt(event.changedTouches[0].clientY);
        console.log("posicionX = ", emperzarToqueX, " y posicionY = ", emperzarToqueY);
    }
);

document.addEventListener( "touchend",
    (event) =>{
        terminarToqueX = parseInt(event.changedTouches[0].clientX);
